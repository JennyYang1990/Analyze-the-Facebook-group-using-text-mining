...
stop = set(stopwords.words('english') + punctuation + ['gamification','#gamification','com','pm','de','el','que','e','un','en','el'])
...
def preprocess(s, lowercase=True):
    ...
    if lowercase:
        ...
        tokens = [tk for tk in tokens if tk.isascii()]
...
def process_data(text_in_list):
    
    data_processed = []
    
    for text in text_in_list:
        tokens = preprocess(text)
        data_processed.append(tokens)

    common_dictionary = Dictionary(data_processed)
    common_corpus = [common_dictionary.doc2bow(text) for text in data_processed]
    
    return common_dictionary, common_corpus

def run_lda(common_corpus, common_dictionary, topic_num=10, display_num=20):
    lda = LdaModel(common_corpus, num_topics=topic_num, alpha='auto')
    topics = []
    for i in range(topic_num):
        topic_tuples = lda.show_topic(i, topn=display_num)
        topic_words = [common_dictionary[int(tp[0])] for tp in topic_tuples]
        topics.append(topic_words)
    return topics, lda

def save_topics(topics, topic_path='topics_of_backtoschool.pkl'):
    pickle.dump(topics, open(topic_path,'wb'))
    return
